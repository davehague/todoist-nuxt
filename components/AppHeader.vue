    ...
    <div class="mx-auto md:px-12 py-4">
      ...
      <div :class="{ hidden: !menuOpen, flex: menuOpen }"
        ...
        <nav v-if="authStore.isAuthenticated" class="flex-1 mb-4 md:mb-0">
          <ul class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6">
            <template v-if="authStore.todoistToken">
              ...
              <li>
                <RouterLink to="/active" @click="closeMenu"
                  class="text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
                  :class="{ 'font-semibold': $route.path === '/active' }">
                  Active Tasks
                </RouterLink>
              </li>
              <li>
                <RouterLink to="/completed" @click="closeMenu"
                  class="text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
        ...
        <div v-else class="flex items-center space-x-4">
          <RouterLink to="/login" class="text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
            Login
          </RouterLink>
        </div>
      </div>
      <div class="hidden md:flex items-center justify-between mt-6">
        ...
        <div class="flex items-center space-x-4">
          <RouterLink to="/profile" class="flex items-center space-x-2">
            <img :src="authStore.user.picture || '/profile.png'" :key="authStore.user.picture" alt="User Picture"
